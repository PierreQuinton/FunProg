...
The objective of this course is to introduce typing through a order theoretical view. We will overview mathematical notions of order theory such as partial orders, lattices, and we will see how typing is inscribed into this perspective.
...
> **Exercise:** Prove that 
> $$(T'\to S') \preceq (T\to S) \hspace{0.4em}\Leftrightarrow \hspace{0.4em} (T\preceq T') \land (S'\preceq S)$$

In other words, if we have a function $f:T\to S$, we can safely substitute it with a function $f':T'\to S'$ if:
1. Output Covariance: $S'\preceq S$. The new function returns something more specific (or the same) than requested.
2. Input Contravariance: $T\preceq T'$. The new function must be able to handle at least everything the original could, meaning its input requirement is broader.
...
> **Exercise:** Determine if the dependent type $L[T]$ of lists of elements of type $T$ is covariant or contravariant.
...
1. Supremum: $a\lor b$ is the supremum of $a$ and $b$ if $a, b\leq a\lor b$, and for all $c$ with $a, b\leq c$, $a\lor b\leq c$. In plain english $a\lor b$ is an upper bound of $a$ and $b$, and is smaller than any other upper bound.
2. Infimum: $a\land b$ is the infimum of $a$ and $b$ if $a\lor b\leq a, b$, and for all $c$ with $c\leq a, b$, $c\leq a\lor b$. In plain english $a\land b$ is a lower bound of $a$ and $b$, and is larger than any other lower bound.
...
> 2. The infimum of two types $T$ and $S$ is the intersection type $S\cap T$. An object of that type is of type $S$ and or type $T$ at the same type.
...
> 2. If we have an arbitrary collection $C$ of types, then the greatest lower bound of $C$ is $\bigcap_{T\in C} T$: an element of that type is of type $T$ for all $T\in C$.
...
We showed that the lattice of types is complete. If we take the supremum of all types, we get the maximal type with respect to the subtyping relation $\preceq$. In math, we call this element the top element, and we denote it by $\top$. In Python, this is the type `Any`.

Similarly, we can take the inifmum of all types, we get the bottom element $\bot$. In Python, this is called `Never`.

> **Exercice:** What is the supremum of the collection of all function types? What is its infimum?
